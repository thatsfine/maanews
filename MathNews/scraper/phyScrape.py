...
def phys_out():
    ...
    # Find the number of total social media shares for each article, located under a specific div. Also do some stripping
    # of whitespace
    for numShares in soup.find_all('div', {"class","info"}):
        shares.append(numShares.contents[2].strip())
    ...
    for i in range(len(dates)):
        dList.append({'title':titleList[i],'url':urList[i],'blurb':blurbs[i],'date':dates[i],'shares':shares[i]})
